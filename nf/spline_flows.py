...
class NSF_CL(nn.Module):
    ...
    def __init__(
        self, dim, K=5, B=3, hidden_dim=8, base_network=MLP
    ):
        super().__init__()
        self.dim = dim
        self.K = K
    ...
    def create_base_networks(self, context):
        if context is not None and self.context_dim != context.shape[-1]:
            self.context_dim = context.shape[-1]
            self.g1 = self.base_network(self.context_dim, (3 * self.K - 1) * self.dim // 2, self.hidden_dim)
            self.g2 = self.base_network(self.context_dim, (3 * self.K - 1) * self.dim // 2, self.hidden_dim)

    def forward(self, x, context):
        self.create_base_networks(context)
        lower, upper = x[:, : self.dim // 2], x[:, self.dim // 2 :]

        if context is not None:
            # context = context
            out = (self.g1(context) * self.f1(lower)).reshape(-1, self.dim // 2, 3 * self.K - 1)
        else:
        ...
        if context is not None:
           # context = context
            out = (self.g2(context) * self.f2(upper)).reshape(-1, self.dim // 2, 3 * self.K - 1)
        else:
